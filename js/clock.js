...
function Clock_Spread()
{
	...
	document.getElementById("clock_inner").style.filter="opacity(100%)";
	Alarm_Clock();
}
	...
	for (i=0;i<alarm_clock_sum;i++)
	{
		if (Set_Width(hour)+":"+Set_Width(minute)==alarm_clock_time[i] && alarm_clock_tag[i]=="ON" && (alarm_clock_frequency[i]=="每天" || alarm_clock_frequency[i]=="临时" || alarm_clock_frequency[i]=="周一至周五" && day>=1 && day<=5 || alarm_clock_frequency[i]=="周六、周日" && (day==0 || day==6)))
		{
			...
			if (alarm_clock_frequency[i]=="临时")
				$.get("http://0.0.0.0/jsonp/alarm_clock/switch",{number:i},function(data)
				{
...
function Alarm_Clock_Event(type)
{
	$.ajax({
		url:"http://0.0.0.0/jsonp/alarm_clock/read",
		dataType:"jsonp",
		success:
		function(data)
		{
			...
			for (i=0;i<number;i++)
			{
				...
				text+=  "<div id='alarm_clock_"+(i+1)+"' align='center' style='height: 100px; width: 60%; margin: 0 auto; margin-top: 25px; background: rgba(255,255,255,0.125)'>"+
						...
						"<font style='font-size: 20px; color: rgba(255,255,255,0.625)'>&nbsp;"+data[i].frequency+"</font>"+"</td>"+
			...
			{
				case "switch":
				case "add": Alarm_Clock_Edit(alarm_clock_number);break;
				case "delete": Alarm_Clock_Delete_Mode();break;
			}
		}});
}
...
{
	$.get("http://0.0.0.0/jsonp/alarm_clock/switch",{number:(alarm_clock_number-1)},function(data)
		{
			console.log(data.status);
			Alarm_Clock_Event("switch");
		},"jsonp");
}
...
{
	$.get("http://0.0.0.0/jsonp/alarm_clock/delete",{number:(alarm_clock_number-1)},function(data)
		{
			console.log(data.status);
			Alarm_Clock_Event("delete");
		},"jsonp");
}
...
{
	$.get("http://0.0.0.0/jsonp/alarm_clock/add",{time:time,frequency:frequency},function(data)
		{
			console.log(data.status);
			Alarm_Clock_Event("add");
		},"jsonp");
}
...
function World_Time()
{
	...
	"<p style='color: rgba(255,255,255,0.75) ;font-size: 20px'>"+
...
function Switch_Face_Behind()
{
	...
	switch (face)
	{
		case 2: World_Time();sec=setInterval(function(){World_Time();},1000);break;
		case 3: Stopwatch();break;
		case 4: Calculagraph();break; 
	}
	document.getElementById("clock_head_left").style.width=-10+20*face+"%";
	document.getElementById("clock_head_right").style.width=90-20*face+"%";
	if (face!=1)
		document.getElementById("FRONT").style.filter="";
	else
		document.getElementById("FRONT").style.filter="opacity(37.5%)";
	if (face!=4)
		document.getElementById("BEHIND").style.filter="";
	else
		document.getElementById("BEHIND").style.filter="opacity(37.5%)";
}
...
function Clock_Fold()
{
	...
	document.getElementById("clock_inner").style.filter="opacity(0%)";
